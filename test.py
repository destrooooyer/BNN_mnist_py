...
mnist = input_data.read_data_sets("/home/albireo/Desktop/mnist", one_hot=True)
...
file = open("weights_loc.txt", 'w')
tmp = qw.loc.eval()
tmp *= 1024
print >> file, tmp.astype(int)
file.close()

file = open("weights_scale.txt", 'w')
tmp = qw.scale.eval()
tmp *= 1024
print >> file, tmp.astype(int)
file.close()

file = open("bias_loc.txt", 'w')
tmp = qb.loc.eval()
tmp *= 1024
print >> file, tmp.astype(int)
file.close()

file = open("bias_scale.txt", 'w')
tmp = qb.scale.eval()
tmp *= 1024
print >> file, tmp.astype(int)
file.close()
...
for i in range(0,200):
    a.append( qb.sample().eval()[0])

print a
na=np.array(a)
print na.sum()/200
na2=na*na;
print np.sqrt(na2.sum()/200-(na.sum()/200)**2)
